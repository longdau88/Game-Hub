...
export default function TrendingGamesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/api/games?sort=mostPlayed&limit=50`, { headers })
      .then((res) => res.json())
      .then((data) => {
    ...
    <div className="min-h-screen py-8">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4">
          <div className="p-3 bg-gradient-to-br from-orange-500 to-red-600 rounded-2xl shadow-xl shadow-orange-500/30">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
          {games.map((game, idx) => (
            <motion.div
              ...
              className="group relative flex flex-col bg-white/50 dark:bg-zinc-900/50 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl overflow-hidden hover:border-orange-400/50 hover:shadow-[0_0_25px_rgba(249,115,22,0.15)] transition-all duration-300"
            >
              <Link href={`/game/play?id=${game.id}`} className="aspect-[4/3] relative overflow-hidden block">
                ...
                <div
                  className="absolute top-2 left-2 w-7 h-7 rounded-full flex items-center justify-center text-xs font-black shadow-lg"
                  style={{
                    background: idx === 0 ? '#f59e0b' : idx === 1 ? '#94a3b8' : idx === 2 ? '#b45309' : 'rgba(0,0,0,0.6)',
                    color: idx < 3 ? '#000' : '#fff'
                  }}
                >
                  {idx + 1}
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/40 backdrop-blur-[2px]">
                  <div className="w-12 h-12 rounded-full bg-orange-500/90 flex items-center justify-center shadow-lg scale-75 group-hover:scale-100 transition-transform">
              ...
              <div className="p-3 flex-1 flex flex-col">
                <Link href={`/game/play?id=${game.id}`}>
                  <h2 className="font-bold text-sm text-zinc-900 dark:text-white line-clamp-1 group-hover:text-orange-500 transition-colors mb-1">{game.title}</h2>
                </Link>
