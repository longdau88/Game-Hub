  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      {trend.every(point => point.sessions === 0 && point.uniquePlayers === 0) ? (
        <p className="py-16 text-center text-sm text-zinc-500">Chưa có dữ liệu trong khoảng thời gian đã chọn.</p>
      ) : (
...
export default function AdminAnalyticsPage() {
  ...
  if (!data) return <p className="py-20 text-center text-zinc-500">Không thể tải dữ liệu thống kê.</p>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><h3 className="text-lg font-semibold">{t("admin.analyticsTitle")}</h3><p className="text-sm text-zinc-500 mt-1">Hiệu suất game và chất lượng trải nghiệm.</p></div>
        <div className="flex flex-wrap gap-2">
          <select aria-label="Khoảng thời gian" value={range} onChange={event => setRange(event.target.value)} className="h-10 rounded-lg border border-border bg-card px-3 text-sm">
            <option value="7d">7 ngày qua</option><option value="30d">30 ngày qua</option><option value="90d">90 ngày qua</option>
          </select>
          <select aria-label="Game" value={gameId} onChange={event => setGameId(event.target.value)} className="h-10 max-w-52 rounded-lg border border-border bg-card px-3 text-sm">
            <option value="">Tất cả game</option>{data.games.map(game => <option key={game.id} value={game.id}>{game.title}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden"><div className="p-4 border-b border-border"><h4 className="font-semibold">Xu hướng sessions & người chơi</h4></div><TrendChart trend={data.trend} /></div>

      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="bg-card border border-border rounded-xl overflow-hidden xl:col-span-2"><div className="p-4 border-b border-border"><h4 className="font-semibold">Top game theo lượt chơi</h4></div><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead className="bg-muted/30 border-b border-border"><tr><th className="p-4 font-medium">Game</th><th className="p-4 font-medium">Sessions</th><th className="p-4 font-medium">Người chơi</th><th className="p-4 font-medium">TB chơi</th><th className="p-4 font-medium">Đánh giá</th><th className="p-4 font-medium">Yêu thích</th></tr></thead><tbody className="divide-y divide-border">{data.topGames.map(game => <tr key={game.gameId} className="hover:bg-muted/20"><td className="p-4 font-medium">{game.gameTitle}</td><td className="p-4">{game.sessions}</td><td className="p-4">{game.uniquePlayers}</td><td className="p-4">{game.averageSessionLength.toFixed(1)} giây</td><td className="p-4">{game.totalRatings ? `${game.averageRating.toFixed(1)} / 5 (${game.totalRatings})` : "-"}</td><td className="p-4">{game.favorites}</td></tr>)}{data.topGames.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-zinc-500">Chưa có game phù hợp.</td></tr>}</tbody></table></div></div>
        <div className="space-y-6"><div className="bg-card border border-border rounded-xl p-5"><h4 className="font-semibold">Chất lượng game</h4><div className="mt-5 space-y-4"><div><p className="text-sm text-zinc-500">Crash rate</p><p className={`mt-1 text-3xl font-bold ${data.summary.crashRate > 5 ? "text-red-500" : "text-emerald-500"}`}>{data.summary.crashRate}%</p><p className="mt-1 text-xs text-zinc-500">{data.summary.crashes} lỗi trên {data.summary.sessions} sessions</p></div><div className="border-t border-border pt-4"><p className="text-sm text-zinc-500">Retention</p><div className="mt-2 grid grid-cols-3 gap-2 text-center"><div><p className="font-semibold">{data.retention.d1}%</p><p className="text-xs text-zinc-500">D1</p></div><div><p className="font-semibold">{data.retention.d7}%</p><p className="text-xs text-zinc-500">D7</p></div><div><p className="font-semibold">{data.retention.d30}%</p><p className="text-xs text-zinc-500">D30</p></div></div></div></div></div><div className="bg-card border border-border rounded-xl p-5"><h4 className="font-semibold">Crash theo game</h4><div className="mt-4 space-y-3">{data.topGames.slice(0, 5).map(game => <div key={game.gameId}><div className="flex justify-between gap-3 text-sm"><span className="truncate">{game.gameTitle}</span><span className="font-medium">{game.crashRate.toFixed(1)}%</span></div><div className="mt-1 h-2 rounded-full bg-muted"><div className="h-2 rounded-full bg-red-500" style={{ width: `${Math.min(game.crashRate, 100)}%` }} /></div></div>)}{data.topGames.length === 0 && <p className="text-sm text-zinc-500">Chưa có dữ liệu.</p>}</div></div></div>
      </div>
    </div>
  );
}
