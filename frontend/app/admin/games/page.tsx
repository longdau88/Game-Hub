          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-muted/30 border-b border-border">
              <tr>
                ...
                <th className="p-4 font-medium text-zinc-400">{t("admin.size")}</th>
                <th className="p-4 font-medium text-zinc-400">Uploader</th>
                <th className="p-4 font-medium text-zinc-400">Featured</th>
            ...
            <tbody className="divide-y divide-border">
              {publishedGames.map((game) => (
                <tr key={game.id} className="hover:bg-muted/20">
                  ...
                  <td className="p-4 text-zinc-400">{formatBytes(Number(game.sizeBytes))}</td>
                  ...
                  <td className="p-4">
                    <button
                      ...
                      className={`px-2 py-1 rounded text-xs font-medium ${game.isFeatured ? "bg-yellow-500/10 text-yellow-500 border border-yellow-500/20" : "bg-zinc-800 text-zinc-400 border border-zinc-700"}`}
                    >
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-8">
              <section>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={syncVector} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded text-sm transition-colors">{t("admin.advSyncBtn")}</button>
                </div>
                <div className="bg-zinc-800/50 p-4 rounded-lg border border-zinc-700/50">
                  <label className="block text-sm text-zinc-400 mb-2">{t("admin.advHiddenTags")}</label>
                  <div className="flex gap-2">
                    <input
                      value={advTags}
              ...
              <section>
                ...
                <div className="bg-zinc-800/50 p-4 rounded-lg border border-zinc-700/50 space-y-4">
                  ...
                  <div className="flex justify-end">
                    <button onClick={saveEngineConfig} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-md transition-colors font-medium">Save Engine Config</button>
                  </div>
                </div>
              </section>
